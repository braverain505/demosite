...
(() => {
  ...
  onReady(() => {
    ...
    [...months.entries()].forEach(([key, list]) => {
      ...
      html += `<h3 class="mt-2">${short(m)} ${y}</h3><div class="cal"><div class="cal-dow">${['M','T','W','T','F','S','S'].map((x) => `<span>${x}</span>`).join('')}</div>`;
      ...
      html += '</div>';
      html += `<ul class="cal-list">${list.map((ev) => `<li><b class="t-num">${ev.date}</b> — ${ev.title} <em class="tiny-label">${ev.tag}</em></li>`).join('')}</ul>`;
    });
    cal.innerHTML = html;
